...
import { courses, official, defaultPlans, weekdays, times, storageKey, conflicts, slotText, restorePlans } from './model.js';
import './style.css';
...
function App() {
  ...
  const preview = detail || hovered;
  const previewConflicts = preview ? conflicts(preview, selected) : [];
  const results = courses.filter(c => !c.official && `${c.name} ${c.id} ${c.teacher}`.toLowerCase().includes(query.trim().toLowerCase()) && (category === '全部課程' || c.category === category) && (!onlyFree || conflicts(c,selected).length === 0));
  ...
  useEffect(() => {
    ...
    function trap(event) {
      ...
      const elements = [...panelRef.current.querySelectorAll('button:not(:disabled), input, select, [tabindex="0"]')];
  ...
  function add(course) {
    if (selectedIds.includes(course.id) || conflicts(course, selected).length) return;
    setPlans(prev => [...prev, course.id]);
    setNotice(`已將「${course.name}」加入預排，尚未送出官方選課。`);
  }
  function remove(course) { setPlans(prev => prev.filter(id => id !== course.id)); setNotice(`已移除「${course.name}」的預排。`); }
  ...
  return <div className={`app ${panel ? 'panel-open' : ''}`}>
    ...
    <div className="shell">
      ...
      <main id="main">
        ...
        <div className="summary"><div><span className="status-dot blue"/>已選上 <strong>18</strong><span>學分</span><small>6 門課</small></div><div><span className="status-dot amber"/>預排中 <strong>{plannedCredits}</strong><span>學分</span><small>{planned.length} 門課</small></div><div className="summary-note"><CheckCircle2 size={16}/>已選與預排分開計算</div></div>
        <div className="workspace">
          <section className="calendar-section" aria-label="本學期課表">
            ...
            <div className="legend"><span><i className="blue"/>本系</span><span><i className="teal"/>雙主修</span><span><i className="purple"/>通識</span><span><i className="orange"/>輔系</span><span className="planned-key">虛線框：預排中</span></div>
            {view === 'week' ? <div className="calendar-scroll"><div className="calendar">
              ...
              {selected.flatMap(course => course.slots.map(([day,start,length],i) => <button key={`${course.id}-${i}`} className={`course-block ${course.color} ${course.official?'official':'planned'} ${detail?.id===course.id?'focused':''} ${preview && preview.id !== course.id && preview.slots.some(([d,s,l]) => d===day && s<start+length && start<s+l)?'conflict-target':''}`} style={{gridColumn:day+2,gridRow:`${start+1} / span ${length}`}} onClick={() => openDetail(course)} aria-label={`${course.name}，${course.official?'已選上':'預排中'}，${slotText(course)}`}><strong>{course.name}</strong>{length>1 && <><span>{course.teacher} · {course.room}</span><small>{course.official?'已選上':'預排中'}</small></>}</button>))}
              {preview && !selectedIds.includes(preview.id) && preview.slots.map(([day,start,length],i) => <div key={`preview-${i}`} className={`course-block preview ${previewConflicts.length?'conflict-preview':''}`} style={{gridColumn:day+2,gridRow:`${start+1} / span ${length}`}}><strong>{preview.name}</strong><small>{previewConflicts.length?'時段衝突':'加入後的位置'}</small></div>)}
            </div></div> : <div className="schedule-list">{selected.map(c => <button key={c.id} className="schedule-row" onClick={() => openDetail(c)}><span className={`course-mark ${c.color}`}><BookOpen size={18}/></span><span><strong>{c.name}</strong><small>{slotText(c)} · {c.room}</small></span><span className={`badge ${c.official?'neutral':'amber'}`}>{c.official?'已選上':'預排中'}</span><ChevronRight size={16}/></button>)}</div>}
            <div className="calendar-footer"><span><Check size={14}/>{storageError?'暫時無法儲存，重整後可能遺失預排':'預排自動儲存在此瀏覽器'}</span><span>點選課程查看詳情</span></div>
            <div className="planning-note"><div className="note-icon"><BookOpen size={20}/></div><div><strong>先試排，再做決定。</strong><p>預排課程只用來比較時間安排，不代表已選上，也不計入已完成學分。</p></div></div>
          </section>
