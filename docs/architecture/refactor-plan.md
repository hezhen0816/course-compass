...
- 已建立 typed planner backfill preview/package 工具 `scripts/preview_typed_planner_backfill.py`，可離線拆解 `public.user_data.content`、輸出 typed table 對帳 counts，並產生本機 raw backup package；不連線、不寫 DB、不把敏感 token 放入 preview。
