...
## 2026-09-09 本機新版接上既有資料

依使用者決定採獨立 `/preview.html` 入口，不設新舊切換按鈕。新版六頁沿用正式登入與資料結構，以唯讀 hook 讀取雲端修課資料；課表沿用既有選課階段與官方快照解讀。已完成學分與含未來規劃的數值分開顯示。當次查證 Windows 後端在 Tailscale 顯示離線，課程 API 暫無法驗證；雲端真實資料待使用者在新 origin 登入。Web lint、型別、build 與 3 項隔離持久化測試通過，實際檢視桌面與 390px 手機空白流程。操作範圍與啟動方式見 README「本機新版預覽」。

## 2026-09-09 完成六頁整體互動設計

獨立原型擴為本學期、找課、畢業規劃、修課紀錄、監控、設定六頁。採共用本機資料，讓匯入→完成學分、找課→追蹤、未來學期配置及筆記可連續操作，手機以底部導覽切換。已通過、修讀中、未通過與規劃意向分開呈現；所有官方操作仍只做情境預覽。完成桌面與手機跨頁驗證；功能範圍及限制集中在原型 README 與 design-qa，未替換正式應用或部署。

## 2026-09-09 完整課程查詢互動原型

核對既有查詢實作後，在獨立原型補入進階篩選、跨學期／跨校、排序匯出、待修清單及詳情的認列與追蹤草稿。官方選課保留為成功／拒絕／不確定結果的可點擊示範，歷史課程不能排入本學期。查詢與課表共用預排；本輪未修改正式應用，完整驗證與限制見原型 `design-qa.md`。

## 2026-09-09 原型加入展開找課

獨立原型新增桌面寬版比較列表，與課表側欄共用搜尋條件及預排狀態；詳情加課後可回到課表繼續比較。手機沿用全螢幕面板。實際驗證紀錄集中於 `prototypes/semester-workspace/design-qa.md`，尚未整合正式 Web。
...
## 2026-09-09 原型保留初選與加退選模式

使用者指出初選需同時放入重疊志願。獨立原型補回模式切換：志願登記允許重疊並排，加退選阻擋新增衝堂；切換不刪既有草稿，志願總量不當成可同時修讀學分。PDF 核對出的「初選登記／抽選後繼續選課」差異記在 `docs/data-contracts/ntust-systems.md`，不擴大修改正式系統。
...
## 2026-09-10 修課紀錄與畢業門檻套上新視覺

拆頁時刻意只搬功能、不動樣式；這一步才把兩頁換成製圖紙底的新設計語言。
數字與判斷邏輯一行沒改，`usePlannerStats` 照舊。

- 修課紀錄：五個彩色統計方塊變成一條 mono 統計帶；學期卡改 sheet 底 + rule 邊，
  課列左緣用類別墨色，未通過那列用朱批紅底＋紅字（這正是 `--color-mark` 保留給「擋路的東西」的用途）。
- 畢業門檻：十宮格磚改成帳目列（名稱＋門檻／進度尺／數字＋百分比），分成共同／系所兩張卡；
  體育學期改成一格一學期的方塊；總學分提到頁首。
- `CoursePlannerWebApp` 依分頁切換底色：已移植的三頁用 `bg-paper`，未移植的仍是舊藍灰，
  否則 sheet 卡片浮不出來（本學期頁之前就一直落在錯的底色上）。

**類別墨色沒有沿用 `semesterCategory`**：那支函式把國文／英文／社會實踐併進其他桶，
而這頁的標籤讀的是 `course.category`，共用會讓顏色與旁邊的字互相矛盾。改成跟著顯示的標籤走。

改完跑了一輪對照複查，把改寫時掉掉的東西補回來：頁面 `<h1>` 與標題句、成績前的「成績」二字、
統計的「門」與「門課」、課卡的 Info 可點提示、認列標籤（被塞進 truncate 單行會被截掉，改回會換行的徽章）、
「自訂必修、選修與學分池」與「門檻完成度」兩句副標、每列的百分比讀數、空狀態的虛線框。
另外修掉三個新引入的問題：通識向度徽章被包在「共同門檻」卡裡（門檻全填 0 就整排消失，改放卡片外）、
表單共用 class 的 `outline-none` 關掉了兩個 select 的鍵盤焦點框、體育方塊在門檻大於 12 時與數字對不上。

可及性：實質內容的次要文字從 `text-ink-3`（對比 3.05:1）改成 `text-ink-2`（5.5:1）；
未修向度徽章同樣提高對比；刪除鍵補 `aria-label` 帶規則名稱；課列焦點框改內縮，
才不會被學期卡的 `overflow-hidden` 裁掉。

驗證：Chrome 擴充在這段期間斷線，改用內建瀏覽器加一份臨時 fixture（八學期、未通過、雙主修認列、
未來規劃、空學期）逐項核對畫面文字與 computed style，確認 token 都解析正確（paper/sheet/good/band）、
向度 title 一字未改、百分比與方格數正確；看完即刪除臨時檔。真實帳號的畫面待 Chrome 恢復後再看一次。

## 2026-09-10 修課軌跡拆成「修課紀錄」＋「畢業門檻」

拆頁前先用 76 個 agent 盤點那一頁：**350 項功能、68 個區塊**，其中 331 項在新設計稿裡找不到對應。
關鍵結論是**不要重寫**：把元件整段搬過去，不重打，功能才不會在轉換中掉。

- `features/history/RecordPage.tsx`：頁首、五格統計、八學期時間軸、課程卡、課程詳情彈窗入口
- `features/thresholds/ThresholdsPage.tsx`：門檻完成度（10 列，照 `data.targets` 過濾 target>0）、
  通識向度 A–F 徽章、雙主修／輔系認列規則（新增表單、兩個清單、刪除、比對顯示）
- 數字一律沿用 `usePlannerStats`，一行沒改，所以拆頁後每個進度都跟拆之前一樣

**命名**：使用者選了「修課紀錄」與「畢業門檻」。原本視覺稿叫「畢業規劃」，但那一頁一門課都沒有，
規劃中／待加簽的課實際列在另一頁，兩個名字會互相打架。

**設計稿裡是錯的、沒照做的**（視覺稿是照使用者真實進度反推畫的，數字與敘述都不能當真）：
門檻只畫 7 列（實際 10 列，本系選修與輔系不能砍）、門檻數字寫死 15/47/43（要讀 `data.targets`）、
進度條分「已通過／修讀中」兩段（`usePlannerStats` 現在分不出來）、「大一上→大四下」學程進度軸
（資料模型沒有學期序）、累計 GPA 3.29（app 從來沒算過平均）、「每學期開的班不多」（沒有跨學期開課統計）。

**拆頁本身製造的新問題，一起解掉**：分頁與 hash 同步（原本 `activePage` 只是 useState，重新整理會掉回
課程查詢；拆成兩頁後來回比對變頻繁，這個成本從一次變每次）、各頁記住自己的捲動位置、
導覽說明與首次使用導覽的文案（後者還寫著「側邊欄會顯示學分統計」，而現在根本沒有側邊欄）。

實機驗證（使用者已登入的 Chrome、真實資料）：八學期與五格統計不變；門檻八格與向度徽章（A B C E 已修）
與拆頁前相同；實際新增一條「課碼 BA、學分池」認列規則，正確比對到成本會計／企業概論／證券管理／
管理與企業倫理四門，再按刪除鍵復原，使用者資料沒留下痕跡；課程詳情彈窗開得出來；
切頁寫入 `#record`／`#thresholds`，瀏覽器上一頁回到前一頁且捲動位置還原到 1400px。

注意：**這一步只拆頁，還沒套新視覺**，兩頁仍是原本的藍灰樣式。

## 2026-09-10 新版設計開工：設計 token ＋「本學期」頁

視覺稿定案（七頁，已用使用者 1151 真實資料驗過）後開始移植正式站。這一步只**新增**
一頁，不動任何既有頁面。

- 設計 token 進 `global.css` 的 `@theme`：製圖紙底 `#E9ECE6`、墨色 `#171C19`、
  朱批紅 `#B23A2C`、六個課程類別墨色，字體 IBM Plex Mono ＋ Noto Sans TC。
  舊頁面仍用原本的藍灰，兩套共存到全部移植完。
- `shared/domain/classPeriods.ts`：節次真實起訖分鐘，從既有的
  `OFFICIAL_SCHEDULE_PERIOD_TIMES` 解析出來，不另外維護第二份時刻表。
- `features/semester/`：課表列高按真實分鐘畫（1.08 px/分），休息時間照比例留白。
  「課表看出來的」三條洞察是**算出來的**不是寫死的：最長空檔、最滿的一天、每天都空的節次。

移植過程中修掉的兩件事：

1. **待加簽的課畫不出來**。已選上的課由校務同步寫進 `data.semesters[推定學期]`，
   待加簽卻在 `data.selectionPlan.courses`，只讀前者就會漏掉。兩邊都要讀；
   名稱同時出現在兩邊的（成本會計）標成「待加簽清單裡重複了」，跟選課工作台的說法一致。
2. **類別跟選課工作台不一致**。校務同步進來的雙主修課常常只標「必修」，工作台一直是
   照課碼的系所代碼比對使用者設定的本系／雙主修／輔系。把那段 `classifyCourseByCode`
   從 `PlanningWorkspace` 抽到 `shared/domain/courseClassification.ts` 讓兩頁共用，
   否則同一門課在兩頁會顯示成不同類別。

實機驗證（使用者已登入的 Chrome、localhost:5173、真實資料）：12 門課含夜間 W A,B
書法藝術與待加簽的管理與企業倫理都正確畫出；點課程方塊開詳情、關閉、點待加簽方塊
看到「還不是你的」說明都實測過；420px 寬切成單日＋日期按鈕。
